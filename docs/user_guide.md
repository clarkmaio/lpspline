...
The `lpspline.datasets` module provides synthetic data generation utilities to quickly prototype and test spline models.

**Load Demo Dataset**

The `load_demo_dataset` function generates a multi-dimensional dataset with various non-linear relationships, perfect for demonstrating additive models.

```python
from lpspline.datasets import load_demo_dataset
X, y = load_demo_dataset(samples=1000)
print(X.columns)
# Output: ['xl', 'xpwl', 'xbs', 'xcyc', 'xfactor']
```

**Load By-Group Dataset**

The `load_by_dataset` function is specifically designed to test the `by` argument, allowing you to generate data where relationships vary across groups.

```python
from lpspline.datasets import load_by_dataset
X, y = load_by_dataset(samples=800, type='cyclic')
```

![LPSpline Diagnostic Plot](_static/demo_plot.png)
